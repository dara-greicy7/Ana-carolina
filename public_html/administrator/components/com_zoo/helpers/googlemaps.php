...
class GooglemapsHelper extends AppHelper {
	...
	public function geoCode($address, $cache = null, $key = '') {
		...
		if (!empty($result->hits)) {
			...
			if (!empty($hit->_geoloc)) {
				$coordinates['lat'] = $hit->_geoloc->lat;
				$coordinates['lng'] = $hit->_geoloc->lng;
			}
	...
	public function queryGeoCoder($address, $key = '') {
		...
		$response = $this->app->http->post('https://places-dsn.algolia.net/1/places/query', json_encode(['params' => 'hitsPerPage=1&query='.urlencode($address)]));
