...
class AppRequirements {
	...
	var $_recommended_extensions = array(
		...
		array('name' => 'Alternative PHP Cache (APC)', 'extension' => 'apc', 'info' => 'An opcode cache, like the Alternative PHP Cache (APC) extension, might significantly improve ZOOs memory footprint. https://www.php.net//manual/en/book.apc.php')
	);
	...
	function _checkRequired() {
		...
		$info 	= 'Zoo requires Joomla 3.2+. Please upgrade your Joomla installation (https://www.joomla.org).';
		$this->_addRequiredResult('Joomla 3.2+', $status, $info);

		// check php
		$status = $this->checkPHP();
		$info 	= 'Zoo requires PHP 5.3.10+. Please upgrade your PHP version (https://www.php.net/).';
