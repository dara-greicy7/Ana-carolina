...
class plgSystemZoolingual extends JPlugin
{
	...
	public static $_menu_items_correct = array();
	...
	function onAfterInitialise()
	{
		...
		// Add rule to build route
		$joomapp = JFactory::getApplication();
		$joomapp::getRouter()->attachBuildRule(array($this, 'buildRule'));
	...
	/**
     * Add rule that remembers which itemid has language segment in url.
     * In this case this itemid will not be fixed in sefbuildroute, it has correct item id
	 */
	public function buildRule(&$router, &$uri)
	{
		$joomapp = JFactory::getApplication();

		if ($joomapp->isSite() && $uri->getVar('option') == 'com_zoo') {
			$lang_codes = JLanguageHelper::getLanguages('lang_code');
			$current_lang = $lang_codes[JFactory::getLanguage()->get('tag')]->sef;
			if (!stristr($uri, "/$current_lang/") && // if it is not related to the current language - do not translate alias
				$uri->getVar('Itemid') && // contains itemid
				!in_array($uri->getVar('Itemid'), self::$_menu_items_correct))
			{
				self::$_menu_items_correct[] = $uri->getVar('Itemid');
			}
		}
	}

	/**
	 * Check valid Itemid for current language
	 */
	public function sefbuildroute($event)
	{
		$path = $event->getParameters();

		$itemid = isset($path['query']['Itemid']) ? $path['query']['Itemid'] : null;
		if ($itemid && !in_array($itemid, self::$_menu_items_correct)) {
			$category_id = JFactory::getApplication()->getMenu()->getParams($itemid)->get('category');
			$new_itemid = $this->_find('category', $category_id) ? $this->_find('category', $category_id)->id : null;

			$path['query']['Itemid'] = ($category_id && $new_itemid) ? $new_itemid : $itemid;
		}
	}

	/**
	 * Overridden method of zoo route helper
	 */
	protected function _find($type, $id)
	{
		$lang = JFactory::getLanguage()->get('tag');

		if (plgSystemZoolingual::$_menu_items == null) {
			...
			plgSystemZoolingual::$_menu_items = array_fill_keys(array('frontpage', 'category', 'item', 'submission', 'mysubmissions'), array());
			foreach ($menu_items as $menu_item) {
				switch (@$menu_item->query['view']) {
					case 'category':
						plgSystemZoolingual::$_menu_items['category'][$menu_item->language][$this->app->parameter->create($menu_item->params)->get('category')] = $menu_item;
						break;
				}
			}
		}
		...
		$itemid = null;
		if (isset(plgSystemZoolingual::$_menu_items[$type][$lang][$id])) {
			$itemid = plgSystemZoolingual::$_menu_items[$type][$lang][$id];
		} elseif (isset(plgSystemZoolingual::$_menu_items[$type]{'*'}[$id])) {
			$itemid = plgSystemZoolingual::$_menu_items[$type]{'*'}[$id];
		}
...
/* 		if ($this->app->lang->checkGroup($item, $config->get('group_elms', ''))){
		...
		if( $this->joomla->isSite() )
		{
			...
			if( count( $sub_translations ) )
			{
				if( array_key_exists($lang, $sub_translations))
				{
					if( strlen($sub_translations[$lang]) )
					{
						$category->getParams()->set('content.subtitle', $sub_translations[$lang] );
					}
				}
			}
	...
	public function itemInit($event, $arguments = array())
	{
		...
		if( $this->joomla->isSite())
		{
			...
			// remember translated items
			if (!empty($translated_items[$item->id]) && $translated_items[$item->id] == $lang) {
				return;
			} else {
				$translated_items[$item->id] = $lang;
			}
