...
class AppAlias {
	...
	public function translateAliasToID($alias) {
		...
		if ($id = $db->queryResult($query)){
			...
			// find unicode format of alias
			$encoded_alias = (string)$this->app->parameter->create(array('encoded_alias' => $alias));
			preg_match('((?:"encoded_alias":)(?: )*(?:.)*(?: )*(?:.)(?: )*"(.*)")', $encoded_alias, $matches);
			$unicoded_alias = $db->escape($matches[1]);

			// if not found, search into translated ones
			$lang = JFactory::getLanguage()->get('tag');
			$server_info = $db->getConnection()->server_info;
			
			if (strstr(strtolower($server_info), 'mariadb')) { // workaround for MariaDB
				$query = 'SELECT id'
					.' FROM '.$this->_table->name
					.' WHERE params REGEXP '.$db->Quote('(*CR)("content.alias_translation":)( )*(.)*( )*("[a-z]*-[A-Z]*":)( )*("'.$alias.'")')
					. ' OR params REGEXP '.$db->Quote('(*CR)("content.alias_translation":)( )*(.)*( )*("[a-z]*-[A-Z]*":)( )*("'.$unicoded_alias.'")')
					.' LIMIT 1';
			    ...
			    $query = 'SELECT id'
					   ...
					   . ' OR params REGEXP '.$db->Quote('(("content.alias_translation":)( )*(.)*( )*("[a-z]*-[A-Z]*":)( )*("'.$unicoded_alias.'"))')
					   .' LIMIT 1';
			}
