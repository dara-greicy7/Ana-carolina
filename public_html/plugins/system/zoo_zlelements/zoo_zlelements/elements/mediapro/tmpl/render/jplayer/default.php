...
$this->app->document->addScript('elements:mediapro/tmpl/render/jplayer/default/script.js');
...
foreach ($media_list['result'] as $index => $media_obj)
{
	...
	switch ($this->getVideoFormat($media_obj->getURL())) {
		...
		default: // case file
			...
			$ext = strtolower($media_obj->getExtension());
			...
			if ($params->find('layout._playlist', 0)) {
				...
				if ($layout = $this->getLayout("render/jplayer/default/layout.php")) {
					$result[] = $this->renderLayout($layout, compact('params', 'id', 'playlist', 'supplied', 'width', 'height'));
				}
			}
	}
}
